...
from crop_service import CameraRegistry, JsonStateStore, _axis_aligned_bbox, _crop_frame
...
def test_state_store_roundtrip(tmp_path: Path) -> None:
    ...
    registry = CameraRegistry(store)
    registry.register("cam-1", "/tmp/video.jpg", {"frame_index": 1})
    registry.update_crop_json("cam-1", {"frame_index": 2})

    reloaded = CameraRegistry(store)
    reloaded.load()
    camera = reloaded.get("cam-1")
